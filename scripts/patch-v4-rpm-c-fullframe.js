...
html=html.replace('</style>',`
 ...
 @media(max-width:430px){.rpmCWrap{min-height:220px}.rpmC{width:min(88vw,390px)}.rpmCSeg{width:9px;height:25px}.rpmCValue{font-size:56px}.nav button,.nav a{font-size:10px!important;padding-top:32px!important}}
 </style>`);
...
(function(){
 ...
 function updateGauge(){
  ...
  const value=document.getElementById('rpmCValue');if(value)value.textContent=Math.round(raw);
  const segs=[...document.querySelectorAll('.rpmCSeg')];
  segs.forEach((s,i)=>{
   s.className='rpmCSeg';
   if(i<on){
    const p=i/(N-1);
    const cls=p<.58?'green':p<.76?'yellow':p<.9?'orange':'red';
...
console.log('RPM em C corrigido: verde na direita, vermelho na esquerda.');
