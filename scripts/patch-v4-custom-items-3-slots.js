...
.customBuilderDialog{width:min(650px,100%);max-height:91vh;overflow:auto;background:#202122;border:1px solid #454545;border-radius:28px 28px 0 0;padding:0 22px calc(92px + env(safe-area-inset-bottom));-webkit-overflow-scrolling:touch;position:relative}
.customBuilderHead{position:sticky;top:0;z-index:3;display:flex;align-items:center;justify-content:space-between;background:#202122;padding:24px 4px 18px;border-bottom:1px solid #414141}
...
.customBuilderActions{position:sticky;bottom:0;z-index:4;display:grid;grid-template-columns:.8fr 1.2fr;gap:10px;margin:26px -22px -92px;padding:14px 22px calc(16px + env(safe-area-inset-bottom));background:linear-gradient(180deg,rgba(32,33,34,.88),#202122 24%);border-top:1px solid #414141;backdrop-filter:blur(10px)}
.customBuilderActions button{border:0;border-radius:16px;padding:17px 12px;font-weight:900;font-size:15px;letter-spacing:.4px}.customCancel{background:#3a3a3a;color:#fff}.customSave{background:linear-gradient(135deg,#4a89f4,#1968ef);color:#fff;box-shadow:0 8px 20px rgba(40,110,240,.3)}
.customSave:active{transform:scale(.98)}
...
@media(max-width:430px){.v4CustomGroup.separate.slots-3 .v4CustomCell{grid-column:span 2}.v4CustomCell{min-height:112px;padding:14px}.v4CustomValue{font-size:28px}.v4CustomLabel{font-size:12px}.customBuilderDialog{padding-left:16px;padding-right:16px}.customBuilderActions{margin-left:-16px;margin-right:-16px;padding-left:16px;padding-right:16px}}
`;
...
<div class="customBuilder" id="customBuilder">
 <div class="customBuilderDialog">
  ...
  <div class="customBuilderActions"><button class="customCancel" id="cancelCustomBuilder">CANCELAR</button><button class="customSave" id="saveCustomBuilder">SALVAR ITEM NOVO</button></div>
 </div>
</div>`;
...
(function(){
 ...
 function tick(){document.querySelectorAll('[data-custom-value]').forEach(e=>{e.textContent=readValue(e.dataset.customValue)});requestAnimationFrame(tick)}
 function createGroup(){const count=Number(q('customSlotCount').value||1),slots=[...document.querySelectorAll('[data-custom-slot]')].slice(0,count).map(s=>s.value),name=q('customGroupName').value.trim()||'Personalizado';groups.push({id:'custom-'+Date.now(),name,mode,slots});save();renderGroups();close();if(typeof showPage==='function')showPage('dash')}
...
console.log('Criador personalizado com botão SALVAR ITEM NOVO aplicado.');
