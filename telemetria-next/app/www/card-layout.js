...
/* V5.3 — todos os cartões podem ser movidos para cima, baixo, esquerda e direita. */
(function () {
  const STORAGE_KEY = 'gt7_v5_card_layout_v1';
  ...
  function closestHorizontal(card, direction) {
    ...
    const candidates = directCards(card.parentElement)
      ...
      .sort((a, b) => {
        ...
        return Math.abs(pa.x - source.x) - Math.abs(pb.x - source.x);
      });
  ...
  function closestVertical(card, direction) {
    ...
    const candidates = directCards(card.parentElement)
      ...
      .sort((a, b) => {
        const pa = center(a.rect);
        const pb = center(b.rect);
        const scoreA = Math.abs(pa.y - source.y) * 4 + Math.abs(pa.x - source.x);
        const scoreB = Math.abs(pb.y - source.y) * 4 + Math.abs(pb.x - source.x);
  ...
  function fallbackSibling(card, direction) {
    ...
    if (direction === 'left' || direction === 'up') return cards[index - 1] || null;
  ...
  function moveCard(card, direction) {
    ...
    if (direction === 'left' || direction === 'right') {
      target = closestHorizontal(card, direction) || fallbackSibling(card, direction);
    } else {
      target = closestVertical(card, direction) || fallbackSibling(card, direction);
    }
