/* Bridge extraída diretamente do APK GT7-Telemetria-V4.08-Bridge-Persistente-debug.apk */
(function(){
 ...
 function adapt(root){
  ...
  const car=L.car||{},input=L.input||{},lap=L.lap||{},fuel=L.fuel||{},legacy=L.legacy||{},tyres=L.tyres||{};
 ...
 async function readLive(){try{return await req('/api/live','GET',null,5000)}catch(first){return await req('/api/fields','GET',null,5000)}}
 async function applyPs5(){const bodies=[['/api/config',{ps5Ip:PS5}],['/api/settings',{ps5Ip:PS5}],['/api/ps5',{ip:PS5}]];for(const [p,b] of bodies){try{await req(p,'POST',b,3000);return true}catch{}}return false}
 async function connect(){
  ...
  try{
   ...
   return true;
  }catch(e){failures++;status('off',e.message||'SEM RESPOSTA',0);return false}
 }
 async function poll(){
  ...
  try{
   ...
   failures++;
   if(failures<8){if(el('bridgeText'))el('bridgeText').textContent='SINAL INSTÁVEL · '+failures+'/8';if(el('statusDot'))el('statusDot').style.background='#ffb000'}
...
(function(){
 ...
 function paint(ok,waiting=false,message='GT7-UDP'){
  ...
  if(q('statusDot')){q('statusDot').style.background=connected?'var(--cyan)':waiting?'#ffb000':'#555';q('statusDot').style.boxShadow=connected?'0 0 18px var(--cyan)':'none'}
  if(q('bridgeText'))q('bridgeText').textContent=(connected?'OK · ':waiting?'CONECTANDO · ':'OFF · ')+message;
 }
 async function json(path,timeout=3500){const ctrl=new AbortController();const tid=setTimeout(()=>ctrl.abort(),timeout);try{const r=await fetch(BRIDGE+path,{cache:'no-store',signal:ctrl.signal,headers:{Accept:'application/json'}});if(!r.ok)throw new Error('HTTP '+r.status);return await r.json()}finally{clearTimeout(tid)}}
 async function live(){try{return await json('/api/live',3200)}catch(e){return await json('/api/fields',3200)}}
 function forceConnectedPayload(raw){const d=(raw&&raw.live)?{...raw.live,session:raw.session||raw.active||raw.live.session}:({...raw});d.connected=true;d.decodeOk=d.decodeOk!==false;return d}
 async function tick(){
  if(stopped||busy)return;busy=true;const start=performance.now();
  try{const raw=await live();const d=forceConnectedPayload(raw||{});lastLatency=Math.max(1,Math.round(performance.now()-start));lastOk=Date.now();failures=0;lastData=d;if(typeof window.render==='function'){try{window.render(d)}catch(err){console.warn('Render não derrubou Bridge:',err)}}paint(true,false,(raw&&raw.app)||'GT7-UDP')}
  catch(err){failures++;paint(false,failures<6,'GT7-UDP')}
  finally{busy=false;clearTimeout(retryTimer);retryTimer=setTimeout(tick,failures?Math.min(2500,700+failures*250):850)}
 }
 async function post(path,data){const r=await fetch(BRIDGE+path,{method:'POST',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify(data||{})});if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}
 function start(){try{if(typeof timer!=='undefined'&&timer){clearInterval(timer);timer=null}}catch{}stopped=false;paint(false,true);tick()}
 function bind(){if(q('connectBtn'))q('connectBtn').onclick=start;if(q('startSection'))q('startSection').onclick=async()=>{await post('/api/session/start',{name:'Nova seção'});tick()};if(q('saveSection'))q('saveSection').onclick=async()=>{const name=prompt('Nome da seção:','Seção '+new Date().toLocaleString('pt-BR'));if(name!==null){await post('/api/session/finish',{name});tick()}};if(q('resetSection'))q('resetSection').onclick=async()=>{await post('/api/reset',{});tick()};document.addEventListener('visibilitychange',()=>{if(!document.hidden){clearTimeout(retryTimer);tick()}});window.addEventListener('online',()=>{clearTimeout(retryTimer);tick()});start()}
